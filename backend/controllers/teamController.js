...
export const getTeamMembers = async (req, res) => {
  try {
    ...
    // L2: Get self + L1 managers + their L0 reports
    else if (userLevel === 2) {
      // Get direct L1 reports
      ...
      // Get L0 employees under each L1 manager
      ...
      // Include self first, then L1 managers, then L0 employees
      teamMembers = [currentUser, ...l1Managers, ...l0Employees];
    }
    // L3: Get all active employees
    else if (userLevel === 3) {
...
export const getTeamStats = async (req, res) => {
  try {
    ...
    if (userLevel === 1) {
      // L1: Query direct L0 reports from database
      const directReports = await User.find({
        reportingManager: req.user.id,
        managementLevel: 0,
        isActive: true
      ...
      // L2: L1 managers + their L0 reports
      const l1Managers = await User.find({
        reportingManager: req.user.id,
        managementLevel: 1,
        isActive: true
      ...
      // Query L0 employees under those L1 managers directly from database
      const l0Employees = await User.find({
        reportingManager: { $in: l1ManagerIds },
        managementLevel: 0,
        isActive: true
      ...
      teamMemberIds = [...l1ManagerIds, ...l0EmployeeIds];
    } else if (userLevel === 3) {
      // L3: All employees except self
      const allUsers = await User.find({
        isActive: true,
        _id: { $ne: req.user.id }
      }).select('_id');
      
      teamMemberIds = allUsers.map(u => u._id);
    }
